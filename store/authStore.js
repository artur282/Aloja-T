    ...
    try {
      ...
      if (session) {
        ...
        let { data: profileById, error: errorById } = await supabase
          .from("users")
          .select(
            "id, email, rol, nombre_completo, numero_telefono, url_foto_perfil, documento_identidad, estado_verificacion"
          )
        ...
        if (!profileById || profileById.length === 0) {
          ...
          if (errorByEmail) {
            console.error(
              "Error al buscar perfil por email:",
          ...
          if (profileByEmail && profileByEmail.length > 0) {
            ...
            console.log(
              "Encontrado perfil con el mismo email pero diferente ID"
            );
            ...
            if (updateError) {
              console.error(
                "Error al actualizar ID del perfil:",
                updateError.message
              );
            } else {
        ...
        if (!profileById || profileById.length === 0) {
          console.log(
            "No se encontró perfil para el usuario, intentando crear uno nuevo..."
          );
          ...
          try {
            ...
            if (insertData && insertData.length > 0) {
              set({
                session,
                user: { ...session.user, ...insertData[0] },
                isLoading: false,
              });
            }
          } catch (insertErr) {
  ...
  updateProfile: async (profileData) => {
    ...
    try {
      ...
      console.log(
        ...
        JSON.stringify({ ...user, ...updatedUserData }, null, 2)
      );
